...
const ImageCarousel = () => {
  ...
  return (
    ...
    {bannerList?.map((item,index) => {
        return (
          <div key={item.id} className={index === current ? "slide active": "slide"} >
              {index === current && (<img src={item.bannerImageUrl} alt={item.bannerImageAlt} />) }
        ...
        {bannerList.map((item,index) => (
          <button
            key={item.id}
            className={ index === current ? "carousel-dots-slider dots-active": "carousel-dots-slider" }
          ></button>
        ))}
      </div>
    </section>
